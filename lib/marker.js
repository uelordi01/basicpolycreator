...
RenderHandler.prototype.setSelectedIndex=function(index)
{
  this.index=index;
}
RenderHandler.prototype.getSelectedIndex=function(index)
{
  return this.index;
}
...
RenderHandler.prototype.setTranslateSensitivity=function(value)
{
  this.scaleFactor=value;
}
RenderHandler.prototype.getTranslateSensitivity=function()
{
...
*/
Shape.prototype.moveTo=function(x,y,scaleFactor)
{
  //calculate gravity center:
  var offsets=this.findMinMaxOfThePolygon();
  //calculate the relative distance to the target point
  var xdesp=(x-this.x);//*scaleFactor;
  var ydesp=(y-this.y);//*scaleFactor;
   console.log("xdesp"+xdesp+" ydesp"+ydesp);
  for(var i=0;i<this.markerList.length;i++)
	{
	   this.markerList[i].setX(this.markerList[i].getX()+xdesp);
     this.markerList[i].setY(this.markerList[i].getY()+ydesp);
	}
  this.x=x;
  this.y=y;
}
Shape.prototype.isMovable=function(x,y, maxX,maxY)
{
  var offset=this.findMinMaxOfThePolygon();
  var xdesp=x-this.x;
  var ydesp=y-this.y;

  var myminx=offset.xmin-xdesp;
  var myminy=offset.ymin-ydesp;
  var mymaxx=xdesp+offset.xmax;
  var mymaxy=ydesp+offset.ymax;

  if(mymaxx > maxX || mymaxy > maxY ||  myminx <  5 || myminy <= 5  )
  {
      return 0;
  }
  else
